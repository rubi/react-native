...
export default class T2 extends Component<{}> {
			...
			<View style={{
				...
				<View style={{flex: 1, flexDirection: 'row', justifyContent: 'flex-start'}}>
					<View style={{flex: 4, justifyContent: 'center'}}>
						<Text style={{textAlign: 'left'}}>{moment(item.startTime).format('YYYY-MM-DDTHH:mm:ss')}-{moment(item.endTime).format('YYYY-MM-DDTHH:mm:ss')}</Text>
					</View>
	...
	onFetch(page = 1, startFetch, abortFetch){
		const pageNum = 15;
		fetch(getDomain()+'/api/avail?page='+page+'&pageNum=' + pageNum)
		.then((response)=>{
